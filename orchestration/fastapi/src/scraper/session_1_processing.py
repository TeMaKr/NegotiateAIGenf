...
def _parse_surrounding_p(
    ...
    members_raw = text_parts[0] if len(text_parts) == 1 else text_parts[1]
    members_raw = (
    ...
    if type == "members":
        return members_raw, None, date_of_upload
    else:
        return None, members_raw, date_of_upload


def parse_data(submission: dict, type: str, document_type: str | None) -> UNEPMetadata:
...
def scrape(
    ...
    for accordion_name, type in accordion_names.items():
        ...
        for submission in data:
            ...
            metadata = parse_data(submission, type, document_type=document_type)
