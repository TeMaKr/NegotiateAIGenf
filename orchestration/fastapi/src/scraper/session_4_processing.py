...
def _parse_surrounding_p(
    ...
    # Determine the members and group of states based on the type
    draft_category = None
    members_raw = text_parts[0]
    if (type == "members" or type == "group") and len(text_parts) > 1:
        members_raw = text_parts[1]
    elif type == "cg_group" and len(text_parts) > 1:
        members_raw = text_parts[2]
        draft_category = text_parts[1]
    elif type == "cg_members" and len(text_parts) > 1:
        members_raw = text_parts[1]
        draft_category = text_parts[2]
    ...
    return members, group_of_states, date_of_upload, draft_category
...
def parse_data(
    ...
    author, group_of_states, date_of_upload, least_option_for_draft_category = (
        _parse_surrounding_p(surrounding_p, type)
    )
    draft_category = kwargs.get("subsection", None)
    if not draft_category:
        draft_category = kwargs.get("section", None)
    if not draft_category:
        draft_category = least_option_for_draft_category
...
def scrape(
    ...
    for accordion_name, type in accordion_names.items():
        ...
        for submission in data:
            if submission.get("accordion_name") != accordion_name:
                continue
